...
export function StaffAccountsPanel({ initialAccounts }: { initialAccounts: StaffAccount[] }) {
  const router = useRouter();
  const [showForm, setShowForm] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [busyId, setBusyId] = useState<string | null>(null);
  const [resettingAccount, setResettingAccount] = useState<StaffAccount | null>(null);
  ...
  async function createAccount(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const body = await response.json() as { account?: StaffAccount; error?: { message?: string } };
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
    <div className="flex flex-col gap-4 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
      <div><p className="text-xs font-semibold uppercase tracking-[.14em] text-emerald-700">Access control</p><h2 className="mt-1 text-lg font-bold text-slate-950">Warehouse and sales accounts</h2><p className="mt-1 text-sm text-slate-500">Warehouse users manage stock. Retail Sales users can record offline sales and collections only.</p></div>
      <button type="button" onClick={() => { setShowForm((current) => !current); setError(null); }} className="h-11 rounded-lg bg-[#174f40] px-5 text-sm font-bold text-white">{showForm ? "Close form" : "Add staff user"}</button>
    </div>
