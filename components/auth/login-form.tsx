...
export function LoginForm({ portal }: { portal: "admin" | "warehouse" }) {
  ...
  const isOperations = portal === "warehouse";
  ...
  return <form action="/api/auth/login" method="post" className="mt-6 space-y-4">
    ...
    <div>
      <label htmlFor={usernameId} className="text-xs font-semibold text-slate-700">Username</label>
        <input id={usernameId} name="username" autoComplete="username" required maxLength={200} placeholder={isOperations ? "Warehouse or sales username" : "Admin username"} className="mt-2 h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 text-sm text-slate-900 outline-none transition focus:border-emerald-700 focus:bg-white focus:ring-4 focus:ring-emerald-100"/>
    </div>
    <div>
    ...
    <button type="submit" className="h-12 w-full rounded-xl bg-[#174f40] text-sm font-bold text-white shadow-sm transition hover:bg-[#123f34]">{isOperations ? "Open operations workspace" : "Open command center"}</button>
  </form>;
}
