...
import { integrationOutbox, inventoryBalances, inventoryTransactions, products, shopifyMappings } from "@/db/schema";
...
export async function getWarehouseFoundationStatus(): Promise<WarehouseFoundationStatus> {
  const empty = { products: 0, mappings: 0, balances: 0, transactions: 0, pendingShopifyUpdates: 0, onlineAllocation: 0, retailStock: 0, bufferStock: 0, physicalStock: 0, damagedStock: 0 };
  ...
  try {
    ...
    const [[productCount], [mappingCount], [balanceCount], [transactionCount], [pendingCount], bucketRows] = await Promise.all([
    ...
    return { configured: true, initialized: true, products: productCount.value, mappings: mappingCount.value, balances: balanceCount.value, transactions: transactionCount.value, pendingShopifyUpdates: pendingCount.value, onlineAllocation, retailStock, bufferStock, physicalStock: onlineAllocation + retailStock + bufferStock + qcStock, damagedStock, error: null };
  } catch (error) {
