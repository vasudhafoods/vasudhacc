...
function buildRecommendations(input: Omit<ExecutiveDashboardData, "recommendations" | "dataGaps" | "generatedAt">): ExecutiveRecommendation[] {
  ...
  if (!sales) recommendations.push({ id: "sales-unavailable", priority: "urgent", area: "Sales", title: "Restore CEO sales visibility", evidence: input.salesError ?? "Shopify order analytics is unavailable.", action: "Confirm read_orders access and the installed Shopify app version." });
  if (sales && trend) {
